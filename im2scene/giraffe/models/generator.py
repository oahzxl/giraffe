...
class Generator(nn.Module):
    ...
    def get_object_existance(self, n_boxes, batch_size=32):
        ...
        object_existence = np.zeros((batch_size, n_boxes))
        for b_idx in range(batch_size):
            n_obj = n_objects[b_idx]
            if n_obj > 0:
                idx_true = np.random.choice(
                    n_boxes, size=(n_obj,), replace=False)
                object_existence[b_idx, idx_true] = True
        object_existence = object_existence.astype(np.bool)
    ...
    def volume_render_image(self, latent_codes, camera_matrices,
        ...
        # Arrange Pixels，在[-1,1]的范围内按照像素均匀生成点
        pixels = arange_pixels((res, res), batch_size,
                               invert_y_axis=False)[1].to(device)
        pixels[..., -1] *= -1.
        # Project to 3D world
        # 这里所有像素点设为深度-1，相机设为原点，然后用相机内参和外参矩阵把他们投影到3d世界中
        ...
        # 这里是生成深度box，之前生成的点初始深度都是-1，step的意思是一条ray生成多少点
        di = depth_range[0] + \
        ...
        for i in range(n_iter):
            if i < n_boxes:  # Object
                # 获得物体坐标p(batch,h*w*d,3)，射线r。这里的i代表的box，d是深度上对采样数
                # p的计算比较特殊，是先计算相机往box发射的射线ray，然后根据深度di在射线上采多个点
                ...
                # Mask out values outside，限制在[-1，1]
                padd = 0.1
                mask_box = torch.all(
